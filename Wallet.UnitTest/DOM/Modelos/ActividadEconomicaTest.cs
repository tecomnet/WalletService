...
public class ActividadEconomicaTest : UnitTestTemplate
{
    ...
    [InlineData(data:
    [
        "1. OK: Full Valid", "Mi Actividad Principal", 1500.75, "Ventas Online", "s3://ruta/archivo_valido.pdf", true,
        new string[] { }
    ])]
    ...
    [
        ...
        9999999999999999.99, "Origen largo que tiene 100 caracteres, justo el máximo permitido.",
    ...
    [InlineData(data:
    [
        "4. ERROR: Nombre null", null, 100.00, "Ventas", "s3://ruta/archivo.pdf", false,
        new string[] { ServiceErrorsBuilder.PropertyValidationRequiredError }
    ])]
    [InlineData(data:
    [
        "5. ERROR: Nombre empty", "", 100.00, "Ventas", "s3://ruta/archivo.pdf", false,
        new string[] { ServiceErrorsBuilder.PropertyValidationRequiredError }
    ])] // Ojo: Si mínimo 1, string.Empty falla como requerido.
    [InlineData(data:
    [
        "6. ERROR: Nombre too long",
        "Este nombre es demasiado largo y tiene más de 100 caracteres. Superamos el límite de cien caracteres por poco.",
        100.00, "Ventas", "s3://ruta/archivo.pdf", false,
    ...
    [InlineData(data:
    [
        "7. ERROR: Ingreso cero", "Ventas", 0.00, "Ventas", "s3://ruta/archivo.pdf", false,
        new string[] { ServiceErrorsBuilder.PropertyValidationZeroInvalid }
    ])] // allowZero: false
    [InlineData(data:
    [
        "8. ERROR: Ingreso negativo", "Ventas", -10.50, "Ventas", "s3://ruta/archivo.pdf", false,
        new string[] { ServiceErrorsBuilder.PropertyValidationNegativeInvalid }
    ])] // allowNegative: false
    [InlineData(data:
    [
        "9. ERROR: Ingreso con 3 decimales", "Ventas", 100.123, "Ventas", "s3://ruta/archivo.pdf", false,
        new string[] { ServiceErrorsBuilder.PropertyValidationDecimalsInvalid }
    ])] // allowedDecimals: 2

    // CASOS DE ERROR PARA ORIGEN RECURSO (string, min 1, max 100)
    [InlineData(data:
    [
        "10. ERROR: OrigenRecurso null", "Ventas", 100.00, null, "s3://ruta/archivo.pdf", false,
        new string[] { ServiceErrorsBuilder.PropertyValidationRequiredError }
    ])]
    [InlineData(data:
    [
        "11. ERROR: OrigenRecurso too long",
        ...
        "s3://ruta/archivo.pdf", false, new string[] { ServiceErrorsBuilder.PropertyValidationLengthInvalid }
    ])]

    // CASOS DE ERROR PARA ARCHIVO AWS (string, min 1, max 500)
    [InlineData(data:
    [
        "12. ERROR: ArchivoAWS null", "Ventas", 100.00, "Ventas", null, false,
        new string[] { ServiceErrorsBuilder.PropertyValidationRequiredError }
    ])]
    [InlineData(data:
    [
        "13. ERROR: ArchivoAWS empty", "Ventas", 100.00, "Ventas", "", false,
        new string[] { ServiceErrorsBuilder.PropertyValidationRequiredError }
    ])]
    [InlineData(data:
    [
        "14. ERROR: ArchivoAWS too long", "Ventas", 100.00,
        "Ventas", // 501 caracteres (ejemplo de una cadena que excede por poco los 500)
    ...
    [
        "15. ERROR: Multiple errors", "", 0.00, null, "", false, new string[]
        {
            ...
            ServiceErrorsBuilder.PropertyValidationZeroInvalid, // Ingreso (cero)
            ServiceErrorsBuilder.PropertyValidationRequiredError, // OrigenRecurso
            ServiceErrorsBuilder.PropertyValidationRequiredError // ArchivoAWS
        }
    ])]
