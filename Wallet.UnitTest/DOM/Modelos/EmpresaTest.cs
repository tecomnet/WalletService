...
// Definición de una cadena de 101 caracteres para probar el límite máximo
    ...
    // ----------------------------------------------------------------------------------------------------------------
    [InlineData("1. OK: Nombre válido (mínimo)", 
        "A", 
        true, new string[] { })]
    [InlineData("2. OK: Nombre válido (completo)", 
        "Empresa de Servicios Globales S.A. de C.V.", 
        true, new string[] { })]
    [InlineData("3. OK: Nombre válido (máximo 100)", 
    ...
    // ----------------------------------------------------------------------------------------------------------------
    [InlineData("4. ERROR: Nombre null (Required)",
        null, 
        false, new string[] { "PROPERTY-VALIDATION-REQUIRED-ERROR" })]
    [InlineData("5. ERROR: Nombre vacío (Required)",
        "", 
        false, new string[] { "PROPERTY-VALIDATION-REQUIRED-ERROR" })]
    [InlineData("6. ERROR: Nombre muy largo (> 100)", 
        NombreTooLong, 
    ...
    )
    {
        try
        {
            ...
            Assert.Equal(nombre, empresa.Nombre);

            // Assert success
            Assert.True(success, userMessage: $"El caso '{caseName}' falló cuando se esperaba éxito.");
        }
