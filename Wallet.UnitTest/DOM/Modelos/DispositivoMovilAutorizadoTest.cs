...
// Constantes de datos
    ...
    // === 1. CASOS DE ÉXITO ===
    [InlineData(data: ["1. OK: Full Valid", "Token-ABC-12345", "UUID-XYZ-9876", "iPhone 15 Pro", "iOS 17.0, Screen 6.1, RAM 8GB", true, new string[] { }])]
    [InlineData(data: ["2. OK: Min length", MinLength, MinLength, MinLength, MinLength, true, new string[] { }])]
    [InlineData(data: ["3. OK: Max length (100)", Max100Chars, Max100Chars, Max100Chars, Max100Chars, true, new string[] { }])]

    // === 2. ERRORES DE REQUERIMIENTO (null o empty) ===
    [InlineData(data: ["4. ERROR: Token null", null, "id_valido", "Nombre", "Caracteristicas", false, new string[] { "PROPERTY-VALIDATION-REQUIRED-ERROR" }])]
    [InlineData(data: ["5. ERROR: IdDispositivo empty", "Token", "", "Nombre", "Caracteristicas", false, new string[] { "PROPERTY-VALIDATION-REQUIRED-ERROR" }])]
    [InlineData(data: ["6. ERROR: Nombre null", "Token", "id_valido", null, "Caracteristicas", false, new string[] { "PROPERTY-VALIDATION-REQUIRED-ERROR" }])]
    [InlineData(data: ["7. ERROR: Caracteristicas empty", "Token", "id_valido", "Nombre", "", false, new string[] { "PROPERTY-VALIDATION-REQUIRED-ERROR" }])]

    // === 3. ERRORES DE LONGITUD (> 100 caracteres) ===
    [InlineData(data: ["8. ERROR: Token too long", Max101Chars, "id_valido", "Nombre", "Caracteristicas", false, new string[] { "PROPERTY-VALIDATION-LENGTH-INVALID" }])]
    [InlineData(data: ["9. ERROR: IdDispositivo too long", "Token", Max101Chars, "Nombre", "Caracteristicas", false, new string[] { "PROPERTY-VALIDATION-LENGTH-INVALID" }])]
    [InlineData(data: ["10. ERROR: Nombre too long", "Token", "id_valido", Max101Chars, "Caracteristicas", false, new string[] { "PROPERTY-VALIDATION-LENGTH-INVALID" }])]
    [InlineData(data: ["11. ERROR: Caracteristicas too long", "Token", "id_valido", "Nombre", Max101Chars, false, new string[] { "PROPERTY-VALIDATION-LENGTH-INVALID" }])]
    ...
    )
    {
        try
        {
            ...
            Assert.True(condition: dispositivoAutorizado.Actual, userMessage: "La propiedad 'Actual' debería ser true al crearse.");
        }
