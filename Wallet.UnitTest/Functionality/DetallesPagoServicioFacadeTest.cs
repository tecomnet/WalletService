...
public class DetallesPagoServicioFacadeTest : BaseFacadeTest<IDetallesPagoServicioFacade>, IDisposable
{
    ...
    public DetallesPagoServicioFacadeTest() : base(setupConfig: new SetupDataConfig())
    {
        ...
        _proveedorFacadeMock = new Mock<IProveedorFacade>();
        _facade = new DetallesPagoServicioFacade(context: Context,
            bitacoraTransaccionFacade: _bitacoraTransaccionFacadeMock.Object,
            productoFacade: _proveedorFacadeMock.Object);
    }
    ...
    private async Task<(CuentaWallet wallet, BitacoraTransaccion transaccion)> SetupDataAsync()
    {
        ...
        var usuario = new Usuario(codigoPais: "+52", telefono: "5588888888", correoElectronico: null, contrasena: null,
        ...
        cliente.AgregarDatosPersonales(nombre: "Test", primerApellido: "User", segundoApellido: "Detalles",
            fechaNacimiento: new DateOnly(year: 1990, month: 1, day: 1), genero: Genero.Femenino,
            modificationUser: _userId);
        Context.Cliente.Add(entity: cliente);
        await Context.SaveChangesAsync();

        var wallet = new CuentaWallet(idCliente: cliente.Id, moneda: "MXN", cuentaCLABE: "876543210987654321",
            creationUser: _userId);
        Context.CuentaWallet.Add(entity: wallet);
        await Context.SaveChangesAsync();

        var transaccion = new BitacoraTransaccion(cuentaWalletId: wallet.Id, monto: 200m, tipo: "SERVICIO",
            direccion: "Cargo", estatus: "Completada", creationUser: _userId);
    ...
    public async Task GuardarDetallesAsync_ShouldSaveDetails_WhenValid()
    {
        ...
        // Act
        var result = await _facade.GuardarDetallesAsync(idTransaccion: data.transaccion.Id, idProducto: 101,
            numeroReferencia: "REF_CFE_123", creationUser: _userId, codigoAutorizacion: "AUTH_999");
    ...
    public async Task ObtenerPorClienteAsync_ShouldReturnDetails_ForSpecificClient()
    {
        // Arrange
        var data = await SetupDataAsync();
        await _facade.GuardarDetallesAsync(idTransaccion: data.transaccion.Id, idProducto: 102,
            numeroReferencia: "REF_TELMEX", creationUser: _userId, codigoAutorizacion: "AUTH_888");
    ...
    public async Task RegistrarPagoServicioAsync_ShouldCreateTransactionAndDetails()
    {
        ...
        var realTransaccion = new BitacoraTransaccion(cuentaWalletId: data.wallet.Id, monto: 500m, tipo: "SERVICIO",
            direccion: "Cargo", estatus: "Completada", creationUser: _userId);
        Context.BitacoraTransaccion.Add(entity: realTransaccion);
        await Context.SaveChangesAsync();

        var expectedTransaccion = new BitacoraTransaccion(cuentaWalletId: data.wallet.Id, monto: 500m, tipo: "SERVICIO",
            direccion: "Cargo", estatus: "Completada",
            creationUser: _userId);
        // Use the ID of the real transaction we just created
        typeof(BitacoraTransaccion).GetProperty(name: "Id")
            ?.SetValue(obj: expectedTransaccion, value: realTransaccion.Id);
        ...
        var savedDetail =
            await Context.DetallesPagoServicio.FirstOrDefaultAsync(predicate: d =>
                d.BitacoraTransaccionId == result.Id);
